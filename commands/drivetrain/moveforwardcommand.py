...
class MoveForwardCommand(CommandBase):
    ...
    def isFinished(self):
        ...
        if self.calculateDistanceToFinalPose() <= self.distanceTolerance:
            return True
        elif self.calculateDistanceToFinalPose() > self.slowdownDistance and self.enteredSlowdownRange:
            return True
        return False
    ...
    def calculateDistanceToFinalPose(self):
        distance = self.getRobotPose().translation().distance(self.finalPosition.translation())
    ...
    def calculateFinalRobotPose(self):
        ...
        x_del, y_del = distance * math.cos(self.getRobotAngle()), distance * math.sin(self.getRobotAngle())
        
        finalPose= Pose2d(self.initialPosition.X() + x_del, self.initialPosition.Y() + y_del, self.getRobotAngle())
